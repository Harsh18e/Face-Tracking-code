            ...
            for filename in os.listdir(videos_dir):
                ...
                while True:
                    ...
                    for d in trackers:
                        if not no_display:
                            d = d.astype(np.int32)
                            cv2.rectangle(frame, (d[0], d[1]), (d[2], d[3]), colours[d[4] % 32, :] * 255, 1)
                            savefile_path = os.path.join(frames_path, f'F{c}_ID{d[4]}.jpeg')
                            # print
                            savefile_img = frame[d[1]:d[3], d[0]:d[2]]
                            cv2.imwrite(savefile_path, savefile_img)
                            if final_faces != []:
                                cv2.putText(frame, 'ID : %d ' % (d[4]), (d[0] - 10, d[1] - 10),
                                            cv2.FONT_HERSHEY_SIMPLEX,
                                            0.2,
                                            colours[d[4] % 32, :] * 255, 1)
                                cv2.putText(frame, '', (5, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.2,
                                            (1, 1, 1), 1)
                            else:
